...
4. **規則引擎做了幾個簡化**（詳見 `shared/engine/game.js` 開頭註解）：
   ...
   - **究極（Ultimate）**的特殊召喚簡化為「犧牲自己場上N張卡片」，實際官方條件可能更精細
     （例如限定系統／等級），示範資料先用最單純的版本呈現機制。
5. **已實作的進階系統**：轉醒（Awakening，卡片正反面自動翻轉）、煌臨（Kourin，疊放召喚）、
   究極（Ultimate，特殊召喚卡片類型）、契約（Contract，獨立於主卡組40張之外、開局直接進手牌）。
   詳見 `shared/engine/game.js` 開頭註解與 `shared/data/cards.json` 的 `DEMO-035`〜`DEMO-039`。
6. **AI 是簡單的規則式 AI**（`shared/engine/ai.js`），不是搜尋樹或機率計算，強度有限，且目前
   不會主動使用煌臨／究極特殊召喚（只會一般召喚），這些機制主要是給人類玩家使用/測試。
...
目標卡片即可疊放；究極卡片會出現「特殊召喚（究極）」按鈕，點選需要犧牲的場上卡片數量後
按「確認特殊召喚」；轉醒不需要手動操作，條件成立時場上卡片會自動翻面並標示「已轉醒」。
...
- AI vs AI 模擬對局可以正常跑完、分出勝負，多次重跑沒有出現無限迴圈或例外；轉醒／煌臨／究極
  都有對應的自動化測試（`server/scripts/simulate.mjs`）覆蓋，包含手動觸發與真實對局中自然
  觸發兩種情境。
- 瀏覽器端完整跑過：卡組編輯→匯入預組→開局→出牌（含負擔不起會正確灰掉）→推進步驟→攻擊→
  對手攔截決策，多回合下來沒有 console 錯誤；契約卡開局直接進手牌並顯示標籤；轉醒背面卡
  預設從卡片資料庫隱藏（可勾選顯示，但不能加入卡組）；煌臨與究極特殊召喚的「選擇目標」互動
  （點選按鈕→點選場上卡片→確認）皆已在真實瀏覽器中操作驗證成功。
